...
function ShowLayer(Ly, Color, Weight) {
    ...
    // Re-enable pointer events on all child paths
    if (Ly.eachLayer) {
        Ly.eachLayer(function(FL) {
...
function initializeMap(mapName, registryDetailed, registryFull, registryPresent, stationsDetailed, allNodes, modes, basemapLayerNames, lineMappingJsDetailed, lineMappingJsFull, lineMappingJsPresent, infoPoints, stationSearchIndex) {
    MAP_NAME = mapName;
    ...
    eval(lineMappingJsPresent);

    RegistryDetailed.forEach(L => {
        var Ly = window[L.Id];
        if (Ly) HideLayer(Ly);
    });
    RegistryPresent.forEach(L => {
        var Ly = window[L.Id];
        if (Ly) HideLayer(Ly);
    });
